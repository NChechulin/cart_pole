...
def build_trajectory(config, initial_state, sample_n=100, duration=5):
    ...
    program.AddBoundingBoxConstraint(
        initial_state.as_array(), initial_state.as_array(), program.initial_state()
    )
...
class Trajectory:
    def __init__(self, config, initial_state, sample_n=100, max_duration=5):
        states, targets = build_trajectory(
            config, initial_state, sample_n=100, max_duration=5
        )
        self.states = states
    ...
    def sammple(self, sample_n):
        time_steps = numpy.linspace(
            self.states.start_time(), self.states.end_time(), sample_n
        )
